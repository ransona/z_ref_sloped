function zref(animalID,ref_name,reg_only,fileID)
if ~exist('animalID')
    error('No animal ID')
end
...
try
    if reg_only == false
        ...
        ref_stack_z = linspace(0,fast_z_step,num_ref_planes);
        ref_stack_z = ref_stack_z(1:end-1);
    ...
    all_registered_depths_frames = all_registered_depths_frames - min(all_registered_depths_frames(:));
    all_registered_depths_frames = all_registered_depths_frames / max(all_registered_depths_frames(:));
    all_registered_depths_frames = all_registered_depths_frames * 5000;
    all_registered_depths_frames_sorted = all_registered_depths_frames(:,:,i);
    
    % remove the flyback frames (top ones of each 6 depth stack)
    % leaving only the 5 valid depths
    slices_to_remove = length(ref_stack_z);
    all_registered_depths_sorted = all_registered_depths_sorted(slices_to_remove+1:end);
    all_registered_depths_frames_sorted = all_registered_depths_frames_sorted(:,:,slices_to_remove+1:end);
